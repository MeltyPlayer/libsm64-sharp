namespace libsm64sharp {
  public interface ISm64Triangle {
    ...
    IReadOnlyList<ISm64Vector3<short>> Vertices { get; }
  }
  ...
  public enum Sm64SurfaceType : short {
    ...
    SURFACE_BURNING =
        0x0001, // Lava / Frostbite (in SL), but is used mostly for Lava
    ...
    SURFACE_WATER =
        0x000D, // Water, has no action, used on some waterboxes below
    SURFACE_FLOWING_WATER = 0x000E, // Water (flowing), has parameters

    SURFACE_INTANGIBLE =
        0x0012, // Intangible (Separates BBH mansion from merry-go-round, for room usage)
    ...
    SURFACE_MGR_MUSIC =
        0x001A, // Plays the Merry go round music, see handle_merry_go_round_music in bbh_merry_go_round.inc.c for more details

    SURFACE_INSTANT_WARP_1B =
        0x001B, // Instant warp to another area, used to warp between areas in WDW and the endless stairs to warp back

    SURFACE_INSTANT_WARP_1C =
        0x001C, // Instant warp to another area, used to warp between areas in WDW

    SURFACE_INSTANT_WARP_1D =
        0x001D, // Instant warp to another area, used to warp between areas in DDD, SSL and TTM

    SURFACE_INSTANT_WARP_1E =
        0x001E, // Instant warp to another area, used to warp between areas in DDD, SSL and TTM
    SURFACE_SHALLOW_QUICKSAND = 0x0021, // Shallow Quicksand (depth of 10 units)

    SURFACE_DEEP_QUICKSAND =
        0x0022, // Quicksand (lethal, slow, depth of 160 units)
    ...
    SURFACE_WALL_MISC =
        0x0028, // Used for some walls, Cannon to adjust the camera, and some objects like Warp Pipe
    ...
    SURFACE_THI3_WALLKICK =
        0x0068, // Surface where there's a wall kick section in THI 3rd area, has no action defined

    SURFACE_CAMERA_8_DIR =
        0x0069, // Surface that enables far camera for platforms, used in THI

    SURFACE_CAMERA_MIDDLE =
        0x006E, // Surface camera that returns to the middle, used on the 4 pillars of SSL

    SURFACE_CAMERA_ROTATE_RIGHT =
        0x006F, // Surface camera that rotates to the right (Bowser 1 & THI)

    SURFACE_CAMERA_ROTATE_LEFT =
        0x0070, // Surface camera that rotates to the left (BOB & TTM)

    SURFACE_CAMERA_BOUNDARY =
        0x0072, // Intangible Area, only used to restrict camera movement
    ...
    SURFACE_NO_CAM_COLLISION_77 =
        0x0077, // Surface with no cam collision flag, unused

    SURFACE_NO_CAM_COL_VERY_SLIPPERY =
        0x0078, // Surface with no cam collision flag, very slippery with noise (THI)

    SURFACE_NO_CAM_COL_SLIPPERY =
        0x0079, // Surface with no cam collision flag, slippery with noise (CCM, PSS and TTM slides)

    SURFACE_SWITCH =
        0x007A, // Surface with no cam collision flag, non-slippery with noise, used by switches and Dorrie

    SURFACE_VANISH_CAP_WALLS =
        0x007B, // Vanish cap walls, pass through them with Vanish Cap
    ...
    SURFACE_PAINTING_WOBBLE_BC =
        0x00BC, // Painting wobble (Unused, DDD - Middle)

    SURFACE_PAINTING_WOBBLE_BD =
        0x00BD, // Painting wobble (Unused, DDD - Right)
    ...
    SURFACE_PAINTING_WOBBLE_C8 =
        0x00C8, // Painting wobble (Unused, TTC - Middle)

    SURFACE_PAINTING_WOBBLE_C9 =
        0x00C9, // Painting wobble (Unused, TTC - Right)
    SURFACE_PAINTING_WOBBLE_CA = 0x00CA, // Painting wobble (Unused, SL - Left)

    SURFACE_PAINTING_WOBBLE_CB =
        0x00CB, // Painting wobble (Unused, SL - Middle)
    ...
    SURFACE_PAINTING_WOBBLE_D1 =
        0x00D1, // Painting wobble (Unused, HMC & COTMC - Middle)

    SURFACE_PAINTING_WOBBLE_D2 =
        0x00D2, // Painting wobble (Unused, HMC & COTMC - Right)
