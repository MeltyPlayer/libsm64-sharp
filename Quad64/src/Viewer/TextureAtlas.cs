...
class TextureAtlas {
  ...
  private AtlasEntry
      newEntry(uint texID, int x, int y, int width, int height) {
  ...
  private void buildAtlas(ref List<Bitmap> textures, int width, int height) {
    ...
    foreach (AreaRow r in texRows) {
      ...
      foreach (FilledTex ft in r.FilledSpaces) {
        entries.Add(newEntry(ft.actualID, ft.rect.X, ft.rect.Y, ft.rect.W,
                             ft.rect.H));
  ...
  private void insertIntoEmptySpace(ref AreaRow row,
                                    ref Bitmap bmp,
                                    int emptySpaceIndex,
                                    uint newLocalID) {
