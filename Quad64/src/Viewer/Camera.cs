...
class Camera {
  ...
  public float Yaw {
    get { return CamAngleX; }
  }

  public float Pitch {
    get { return CamAngleY; }
  }

  public float Yaw_Degrees {
    get { return CamAngleX * (180.0f / 3.1415926535897f); }
  }

  public float Pitch_Degrees {
    get { return CamAngleY * (180.0f / 3.1415926535897f); }
  }
  ...
  public void setRotationFromLookAt() {
    ...
    float dist =
        (float) Math.Sqrt(x_diff * x_diff + y_diff * y_diff +
                          z_diff * z_diff);
  ...
  public void updateOrbitCamera(ref Matrix4 cameraMatrix) {
    if (camMode == CameraMode.ORBIT) {
      ...
      if (obj == null)
        return;
      pos.X = obj.xPos +
              (float) (Math.Cos(orbitPhi) * -Math.Sin(orbitTheta) *
                       orbitDistance);
      pos.Y = obj.yPos + (float) (-Math.Sin(orbitPhi) * orbitDistance);
      pos.Z = obj.zPos +
              (float) (Math.Cos(orbitPhi) * Math.Cos(orbitTheta) *
                       orbitDistance);
      lookat.X = obj.xPos;
      lookat.Y = obj.yPos;
      lookat.Z = obj.zPos;
      cameraMatrix = Matrix4.LookAt(pos.X, pos.Y, pos.Z, lookat.X, lookat.Y,
                                    lookat.Z, 0, 1, 0);
      setRotationFromLookAt();
    }
  }
  ...
  public void updateCameraMatrixWithMouse(int mouseX,
                                          int mouseY,
                                          ref Matrix4 cameraMatrix) {
  ...
  public void updateCameraOffsetWithMouse(Vector3 orgPos,
                                          int mouseX,
                                          int mouseY,
                                          int w,
                                          int h,
    ...
    if (camMode == CameraMode.ORBIT && Globals.item_selected > -1)
      ...
      updateCameraOffsetWithMouse_FLY(orgPos, mouseX, mouseY, w, h,
                                      ref cameraMatrix);
  }

  public void updateCameraMatrixWithScrollWheel(
      int amt,
      ref Matrix4 cameraMatrix) {
    if (camMode == CameraMode.ORBIT && Globals.item_selected > -1)
  ...
  private void updateCameraMatrixWithScrollWheel_LOOK(
      int amt,
      ref Matrix4 cameraMatrix) {
    offsetCam(amt, amt, amt);
    orientateCam(CamAngleX, CamAngleY);
  ...
  private void updateCameraMatrixWithMouse_LOOK(
      Vector3 orgPos,
      int mouseX,
      int mouseY,
      ref Matrix4 cameraMatrix) {
    ...
    switch (currentLookDirection) {
      case LookDirection.TOP:
        pos.X = orgPos.X -
                ((MousePosX * Globals.camSpeedMultiplier) * (CamLX) * m) -
                ((MousePosY * Globals.camSpeedMultiplier) * (CamLZ) * m);
        pos.Z = orgPos.Z -
                ((MousePosX * Globals.camSpeedMultiplier) * (CamLZ) * m) -
                ((MousePosY * Globals.camSpeedMultiplier) * (CamLX) * m);
        cameraMatrix = Matrix4.LookAt(pos.X, pos.Y, pos.Z, pos.X,
                                      pos.Y - 1000, pos.Z - 1, 0, 1, 0);
      ...
      case LookDirection.LEFT:
        pos.X = orgPos.X -
                ((MousePosX * Globals.camSpeedMultiplier) * (CamLX) * m);
        pos.Y = orgPos.Y -
                ((MousePosY * Globals.camSpeedMultiplier) * (-1f) * m);
        pos.Z = orgPos.Z -
                ((MousePosX * Globals.camSpeedMultiplier) * (CamLZ) * m);
        cameraMatrix = Matrix4.LookAt(pos.X, pos.Y, pos.Z, pos.X + 12500,
                                      pos.Y, pos.Z, 0, 1, 0);
        lookPositions[(int) currentLookDirection] = pos;
        break;
      case LookDirection.RIGHT:
      ...
      case LookDirection.FRONT:
        ...
        pos.Z = orgPos.Z -
                ((MousePosX * Globals.camSpeedMultiplier) * (CamLZ) * m);
        cameraMatrix = Matrix4.LookAt(pos.X, pos.Y, pos.Z, pos.X, pos.Y,
                                      pos.Z - 12500, 0, 1, 0);
        lookPositions[(int) currentLookDirection] = pos;
        break;
      case LookDirection.BACK:
  ...
  private void updateCameraMatrixWithMouse_FLY(
      int mouseX,
      int mouseY,
      ref Matrix4 cameraMatrix) {
    if (resetMouse) {
  ...
  private void updateCameraOffsetWithMouse_FLY(
      Vector3 orgPos,
      int mouseX,
      int mouseY,
      int w,
    ...
    float CamLZ = (float) -Math.Cos(yaw);
    pos.X = orgPos.X -
            ((MousePosX * Globals.camSpeedMultiplier) * (CamLX) * 6f);
    pos.Y = orgPos.Y -
            ((MousePosY * Globals.camSpeedMultiplier) * (-1f) * 6f);
    pos.Z = orgPos.Z -
            ((MousePosX * Globals.camSpeedMultiplier) * (CamLZ) * 6f);
  ...
  public void updateCameraOffsetDirectly(int horz_amount,
                                         int vert_amount,
                                         ref Matrix4 cameraMatrix) {
    //Console.WriteLine(MousePosX+","+ MousePosY);
    double pitch = CamAngleY - (Math.PI / 2);
  ...
  private void updateCameraMatrixWithMouse_ORBIT(
      int mouseX,
      int mouseY,
      ref Matrix4 cameraMatrix) {
    updateCameraOffsetWithMouse_ORBIT(mouseX, mouseY, ref cameraMatrix);
  ...
  private void updateCameraOffsetWithMouse_ORBIT(
      int mouseX,
      int mouseY,
      ref Matrix4 cameraMatrix) {
    if (resetMouse) {
  ...
  private void updateCameraMatrixWithScrollWheel_ORBIT(
      int amt,
      ref Matrix4 cameraMatrix) {
    orbitDistance -= amt;
