...
namespace Quad64 {
  partial class MainForm : Form {
    ...
    private void initTreeView() {
      ...
      treeView1.Nodes.Add(
          makeTreeNode("3D Objects", Theme.MAIN_TREEVIEW_3DOBJECTS));
      treeView1.Nodes.Add(
          makeTreeNode("Macro 3D Objects", Theme.MAIN_TREEVIEW_MACRO));
      treeView1.Nodes.Add(
          makeTreeNode("Special 3D Objects", Theme.MAIN_TREEVIEW_SPECIAL));
    ...
    void Handle_KeyDown(object sender, KeyEventArgs e) {
      switch (e.KeyCode) {
        case Keys.F1:
          SaveSelectedObjectsToFile(1);
          break;
        case Keys.F2:
          SaveSelectedObjectsToFile(2);
          break;
        case Keys.F3:
          SaveSelectedObjectsToFile(3);
          break;
        case Keys.F4:
          SaveSelectedObjectsToFile(4);
          break;
        case Keys.F5:
          LoadObjectsFromFile(1);
          break;
        case Keys.F6:
          LoadObjectsFromFile(2);
          break;
        case Keys.F7:
          LoadObjectsFromFile(3);
          break;
        case Keys.F8:
          LoadObjectsFromFile(4);
          break;
      }
    }
        ...
        if (items[i] is ToolStripMenuItem) {
          if (((ToolStripMenuItem) items[i]).DropDownItems.Count > 0) {
            ToolStripItemCollection children =
                ((ToolStripMenuItem) items[i]).DropDownItems;
    ...
    private void ToolStripSeparator_Paint(object sender, PaintEventArgs e) {
      ...
      e.Graphics.FillRectangle(new SolidBrush(sep.BackColor), 0, 0, sep.Width,
                               sep.Height);
      e.Graphics.DrawLine(new Pen(sep.ForeColor), 30, sep.Height / 2,
                          sep.Width - 4, sep.Height / 2);
    }
    ...
    private void updateTheme() {
      ...
      treeView1.Nodes[0].ForeColor =
          Theme.MAIN_TREEVIEW_3DOBJECTS; // "3D Objects" text
      treeView1.Nodes[1].ForeColor =
          Theme.MAIN_TREEVIEW_MACRO; // "Macro 3D Objects" text
      treeView1.Nodes[2].ForeColor =
          Theme.MAIN_TREEVIEW_SPECIAL; // "Special 3D Objects" text
    ...
    private void loadROM(bool startingUp) {
      ...
      if (startingUp && !Globals.pathToAutoLoadROM.Equals("")) {
        ...
        openFileDialog1.Filter =
            "Z64 ROM|*.z64|V64 ROM|*.v64|N64 ROM|*.n64|All Files|*";
    ...
    private void selectObject(int mx, int my) {
      ...
      level.getCurrentArea()
           .collision
           .drawCollisionMap(true); // Draw collision map as solid black
      level.getCurrentArea()
           .drawPicking(); // Draw solid color object bounding boxes
      byte[] pixel = new byte[4];
      GL.ReadPixels(mx, h - my, 1, 1, PixelFormat.Rgba, PixelType.UnsignedByte,
                    pixel);
      ...
      if (pixel[2] > 0 && pixel[2] < 4) {
        ...
        if (isControlDown) {
          bool setSelected = !treeViewAlreadyHasNodeSelected(
                                 treeView1.Nodes[Globals.list_selected]
                                          .Nodes[Globals.item_selected]);
          treeView1.ToggleNode(
              treeView1.Nodes[Globals.list_selected]
                       .Nodes[Globals.item_selected],
              setSelected);
          if (setSelected)
            updateAfterSelect(treeView1.Nodes[Globals.list_selected]
                                       .Nodes[Globals.item_selected]);
          else
            treeView1_updateMultiselection();
        } else {
          treeView1.SelectSingleNode(treeView1.Nodes[Globals.list_selected]
                                              .Nodes[Globals.item_selected]);
          updateAfterSelect(treeView1.Nodes[Globals.list_selected]
                                     .Nodes[Globals.item_selected]);
        }
    ...
    private void glControl1_MouseMove(object sender, MouseEventArgs e) {
      if (isMouseDown && e.Button == MouseButtons.Left) {
        if (!isShiftDown && !moveState) {
          ...
          moveState = true;
          camera.updateCameraOffsetWithMouse(savedCamPos, e.X, e.Y,
                                             glControl1.Width,
                                             glControl1.Height, ref camMtx);
        }
        glControl1.Invalidate();
      }
    }
    ...
    private void glControl1_Wheel(object sender, MouseEventArgs e) {
      camera.resetMouseStuff();
      camera.updateCameraMatrixWithScrollWheel((int) (e.Delta * 1.5f),
                                               ref camMtx);
      savedCamPos = camera.Position;
      glControl1.Invalidate();
    }
    ...
    private void glControl1_Resize(object sender, EventArgs e) {
      ...
      ProjMatrix = Matrix4.CreatePerspectiveFieldOfView(
          Globals.FOV * ((float) Math.PI / 180.0f),
          (float) glControl1.Width / (float) glControl1.Height, 100f, 100000f);
    ...
    private void saveROMAsToolStripMenuItem_Click(object sender, EventArgs e) {
      ...
      saveFileDialog1.Filter =
          "Z64 ROM|*.z64|V64 ROM|*.v64|N64 ROM (little endian)|*.n64|N64 ROM (big endian)|*.n64";
    ...
    private void replaceBehavior(int index, ref SelectBehavior behWindow) {
      ...
      treeView1.Nodes[0].Nodes[index].Text =
          area.Objects[index].getObjectComboName();
    }
    ...
    private void replaceObject(int index, ref SelectComboPreset comboWindow) {
      ...
      area.Objects[index]
          .setBehaviorFromAddress(comboWindow.ReturnObjectCombo.Behavior);
      treeView1.Nodes[0].Nodes[index].Text =
          area.Objects[index].getObjectComboName();
    ...
    private void replaceMacroObject(int index,
    ...
    private void replaceSpecialObject(int index,
                                      ref SelectComboPreset comboWindow) {
      //Console.WriteLine(comboWindow.ReturnPresetMacro.PresetID);
      Area area = level.getCurrentArea();
    ...
    private void replaceInstantWarp(int index, ref EditWarp editWarpWindow) {
      ...
      treeView1.Nodes[3].Nodes[index].Text =
          area.InstantWarps[index].ToString();
    }
    ...
    private void objectComboNameToolStripMenuItem_Click(
        object sender,
        EventArgs e) {
    ...
    private void objectComboPresetToolStripMenuItem_Click(
        object sender,
        EventArgs e) {
      ...
      switch (Globals.list_selected) {
        case 0:
          comboWindow =
              new SelectComboPreset(level, 0, "Select Object Combos",
                                    Theme.COMBOS_3DOBJECTS_TITLE);
          ...
          if (comboWindow.ClickedSelect) {
            if (!Globals.isMultiSelected)
              ...
              for (int i = 0; i < treeView1.SelectedNodes.Count; i++)
                replaceObject(treeView1.SelectedNodes[i].Index,
                              ref comboWindow);
          }
          break;
        case 1:
          comboWindow =
              new SelectComboPreset(level, 1, "Select Macro Preset",
                                    Theme.COMBOS_MACRO_TITLE);
          comboWindow.ShowDialog();
          if (comboWindow.ClickedSelect) {
            if (!Globals.isMultiSelected)
              ...
              for (int i = 0; i < treeView1.SelectedNodes.Count; i++)
                replaceMacroObject(treeView1.SelectedNodes[i].Index,
                                   ref comboWindow);
          }
          break;
        case 2: {
          ...
          if (obj.isPropertyShown(Object3D.FLAGS.BPARAM_1))
            ...
            specialListType = 3;
          comboWindow =
              new SelectComboPreset(level, specialListType,
                                    "Select Special Preset",
                                    Theme.COMBOS_SPECIAL_TITLE);
          comboWindow.ShowDialog();
          if (comboWindow.ClickedSelect) {
            if (!Globals.isMultiSelected)
              ...
              for (int i = 0; i < treeView1.SelectedNodes.Count; i++)
                replaceSpecialObject(treeView1.SelectedNodes[i].Index,
                                     ref comboWindow);
          }
          break;
        }
    ...
    private void warpToolStripMenuItem_Click(object sender, EventArgs e) {
      ...
      if (warp is Warp) {
        ...
        EditWarp editWarpWindow = new EditWarp(title, w.WarpFrom_ID,
                                               w.WarpTo_LevelID,
                                               w.WarpTo_AreaID,
                                               w.WarpTo_WarpID);
        ...
        if (editWarpWindow.pressedSaved) {
          if (isPaintingWarp)
            replacePaintingWarp(warpIndex - area.Warps.Count,
                                ref editWarpWindow);
          else
        ...
        EditWarp editWarpWindow =
            new EditWarp(w.TriggerID, w.AreaID, w.TeleX, w.TeleY, w.TeleZ);
        editWarpWindow.ShowDialog();
        if (editWarpWindow.pressedSaved) {
          replaceInstantWarp(
              warpIndex - area.Warps.Count - area.PaintingWarps.Count,
              ref editWarpWindow);
          updateSelectedWarpsInROM();
          glControl1.Invalidate();
          propertyGrid1.Refresh();
    ...
    private void switchLevel(ushort levelID) {
      ...
      if (testLevel.Areas.Count > 0) {
        ...
        ushort id = levelID;
        MessageBox.Show("Error: No areas found in level ID: 0x" +
                        id.ToString("X"));
      }
    }
      ...
      if (node.Parent == null) {
        ...
        if (treeView1.SelectedNodes.Count > 1) {
          ...
          if (Globals.list_selected == 2) {
            Object3D obj3d_0 =
                area.SpecialObjects[treeView1.SelectedNodes[0].Index];
          ...
          for (int i = 1; i < treeView1.SelectedNodes.Count; i++) {
            if (!treeView1.SelectedNodes[i].Parent.Text.Equals(parent_text)) {
              ...
              if (Globals.list_selected != 2 ||
                  (Globals.list_selected == 2 &&
                   Globals.isMultiSelectedFromSpecialObjects))
                break;
            }

            if (treeView1.SelectedNodes[i]
                         .Parent.Text.Equals("Special 3D Objects") &&
                !Globals.isMultiSelectedFromSpecialObjects) {
              Object3D obj3d_i =
                  area.SpecialObjects[treeView1.SelectedNodes[i].Index];
          ...
          Object3D.FLAGS showFlags =
              Object3D.FLAGS.POSITION_X | Object3D.FLAGS.POSITION_Y |
              Object3D.FLAGS.POSITION_Z |
              Object3D.FLAGS.ROTATION_Y | Object3D.FLAGS.BPARAM_1 |
              Object3D.FLAGS.BPARAM_2;
          ;
          ...
          for (int i = 0; i < treeView1.SelectedNodes.Count; i++) {
            ...
            if (local_parent_text.Equals("3D Objects"))
              ...
              selectedObjs[i] =
                  area.MacroObjects[treeView1.SelectedNodes[i].Index];
            } else if (local_parent_text.Equals("Special 3D Objects")) {
              selectedObjs[i] =
                  area.SpecialObjects[treeView1.SelectedNodes[i].Index];
              if (selectedObjs[i].createdFromLevelScriptCommand ==
                  Object3D.FROM_LS_CMD.CMD_2E_8)
                showFlags &= ~(Object3D.FLAGS.BPARAM_1 |
                               Object3D.FLAGS.BPARAM_2 |
                               Object3D.FLAGS.ROTATION_Y);
              else if (selectedObjs[i].createdFromLevelScriptCommand ==
                       Object3D.FROM_LS_CMD.CMD_2E_10)
                showFlags &=
                    ~(Object3D.FLAGS.BPARAM_1 | Object3D.FLAGS.BPARAM_2);
              ...
              else if (treeView1.SelectedNodes[i].Index <
                       area.Warps.Count + area.PaintingWarps.Count)
                selectedWarps[i] =
                    area.PaintingWarps[
                        treeView1.SelectedNodes[i].Index - area.Warps.Count];
              else
                selectedWarps[i] = area.InstantWarps[
                    treeView1.SelectedNodes[i].Index - area.Warps.Count -
                    area.PaintingWarps.Count];
            }
          }
        ...
        if (node.Parent.Text.Equals("3D Objects")) {
          ...
          propertyGrid1.SelectedObject =
              level.getCurrentArea().Objects[node.Index];
          if (camera.isOrbitCamera()) {
            camera.updateOrbitCamera(ref camMtx);
            glControl1.Invalidate();
          }
          ...
          Globals.item_selected = node.Index;
          propertyGrid1.SelectedObject =
              level.getCurrentArea().MacroObjects[node.Index];
          if (camera.isOrbitCamera()) {
            camera.updateOrbitCamera(ref camMtx);
            glControl1.Invalidate();
          }
          ...
          Globals.item_selected = node.Index;
          propertyGrid1.SelectedObject =
              level.getCurrentArea().SpecialObjects[node.Index];
          if (camera.isOrbitCamera()) {
            camera.updateOrbitCamera(ref camMtx);
            glControl1.Invalidate();
          }
          ...
          if (node.Index < area.Warps.Count)
            ...
            propertyGrid1.SelectedObject =
                area.PaintingWarps[node.Index - area.Warps.Count];
          else
            propertyGrid1.SelectedObject = area.InstantWarps[
                node.Index - area.Warps.Count - area.PaintingWarps.Count];
        }
      }
      ...
      if (Globals.list_selected > -1 && Globals.list_selected < 3) {
        if (!Globals.isMultiSelected) {
          ...
          for (int i = 0; i < treeView1.SelectedNodes.Count; i++) {
            ...
            Object3D.FLAGS flag =
                obj.getFlagFromDisplayName(e.ChangedItem.Label);
            if (flag != 0)
    ...
    */
    ...
    private void moveCam_InOut_MouseMove(object sender, MouseEventArgs e) {
      if (moveCam_InOut_mouseDown) {
        camera.resetMouseStuff();
        camera.updateCameraMatrixWithScrollWheel(
            (e.Y - moveCam_InOut_lastPosY) * -10, ref camMtx);
        savedCamPos = camera.Position;
        moveCam_InOut_lastPosY = e.Y;
        glControl1.Invalidate();
      }
          ...
          if (!Globals.isMultiSelected)
            ...
            if (parentText.Equals("3D Objects"))
              return level.getCurrentArea()
                          .Objects[treeView1.SelectedNodes[0].Index];
            else if (parentText.Equals("Macro 3D Objects"))
              return level.getCurrentArea()
                          .MacroObjects[treeView1.SelectedNodes[0].Index];
      ...
      switch (Globals.list_selected) {
        case 3: {
          ...
          if (index < area.Warps.Count) {
            ...
            isPaintingWarp = true;
            propertyGrid1.SelectedObject =
                area.PaintingWarps[index - area.Warps.Count];
          } else {
            propertyGrid1.SelectedObject =
                area.InstantWarps[
                    index - area.Warps.Count - area.PaintingWarps.Count];
          }
        ...
        for (int i = 0; i < 3; i++) {
          foreach (int selectedIndex in Globals.multi_selected_nodes[i]) {
            switch (i) {
              ...
              case 1:
                level.getCurrentArea()
                     .MacroObjects[selectedIndex]
                     .updateROMData();
                break;
              case 2:
                level.getCurrentArea()
                     .SpecialObjects[selectedIndex]
                     .updateROMData();
                break;
            }
          }
    ...
    private void moveObjectXZ(Object3D obj,
                              MouseEventArgs e,
                              Vec3S savedPos,
                              bool forRotation) {
      if (obj == null) return;
      ...
      if (!forRotation) {
        if (obj.isPropertyShown(Object3D.FLAGS.POSITION_X))
          obj.xPos = (short) (savedPos.X -
                              (short) (CX * my * speedMult *
                                       Globals.objSpeedMultiplier) -
                              (short) (CX_2 * mx * speedMult *
                                       Globals.objSpeedMultiplier));
        if (obj.isPropertyShown(Object3D.FLAGS.POSITION_Z))
          obj.zPos = (short) (savedPos.Z -
                              (short) (CZ * my * speedMult *
                                       Globals.objSpeedMultiplier) -
                              (short) (CZ_2 * mx * speedMult *
                                       Globals.objSpeedMultiplier));
          ...
          if (!Globals.isMultiSelected) {
            ...
            for (int i = 0; i < 3; i++) {
              ...
              for (int j = 0; j < Globals.multi_selected_nodes[i].Count; j++) {
                ...
                switch (i) {
                  case 0:
                    moveObjectXZ(level.getCurrentArea().Objects[selectedIndex],
                                 e, moveObj_saved[mo_s_incr++], false);
                    break;
                  case 1:
                    moveObjectXZ(
                        level.getCurrentArea().MacroObjects[selectedIndex], e,
                        moveObj_saved[mo_s_incr++], false);
                    break;
                  case 2:
                    moveObjectXZ(
                        level.getCurrentArea().SpecialObjects[selectedIndex], e,
                        moveObj_saved[mo_s_incr++], false);
                    break;
                }
              }
    ...
    private void moveObjectY(Object3D obj, MouseEventArgs e, bool forRotation) {
      ...
      if (!forRotation) {
        if (obj.isPropertyShown(Object3D.FLAGS.POSITION_Y)) {
          obj.yPos -= (short) (30 * (e.Y - moveObj_UpDown_lastMouseY) *
                               Globals.objSpeedMultiplier);
        }
      } else {
        if (obj.isPropertyShown(Object3D.FLAGS.ROTATION_Y)) {
          obj.yRot -= (short) ((e.Y - rotObj_Yaw_lastMouseY) *
                               Globals.objSpeedMultiplier);
          ...
          if (!Globals.isMultiSelected) {
            ...
            for (int i = 0; i < 3; i++) {
              ...
              for (int j = 0; j < Globals.multi_selected_nodes[i].Count; j++) {
                ...
                switch (i) {
                  case 0:
                    moveObjectY(level.getCurrentArea().Objects[selectedIndex],
                                e, false);
                    break;
                  case 1:
                    moveObjectY(
            ...
            for (int i = 0; i < 3; i++) {
              ...
              for (int j = 0; j < Globals.multi_selected_nodes[i].Count; j++) {
                ...
                switch (i) {
                  case 0:
                    moveObjectXZ(level.getCurrentArea().Objects[selectedIndex],
                                 e, moveObj_saved[mo_s_incr++], true);
                    break;
                  case 1:
                    moveObjectXZ(
                        level.getCurrentArea().MacroObjects[selectedIndex], e,
                        moveObj_saved[mo_s_incr++], true);
                    break;
                  case 2:
                    moveObjectXZ(
                        level.getCurrentArea().SpecialObjects[selectedIndex], e,
    ...
    private void dropObjectToGround() {
      if (Globals.item_selected > -1 && Globals.list_selected > -1) {
        ...
        if (!Globals.isMultiSelected) {
          Object3D obj = getSelectedObject();
          if (obj == null) return;
          obj.yPos = level.getCurrentArea()
                          .collision
                          .dropToGround(
                              new Vector3(obj.xPos, obj.yPos, obj.zPos));
        } else {
          Object3D obj = null;
          for (int i = 0; i < 3; i++) {
            //foreach (int selectedIndex in Globals.multi_selected_nodes[i])
    ...
    private void AreaButton_Click(object sender, EventArgs e) {
      ...
      ushort area =
          ushort.Parse(item.Text.Substring(item.Text.LastIndexOf(" ") + 1));
      trySwitchArea(area);
    }
    ...
    private void
        scriptsDumpToolStripMenuItem_Click(object sender, EventArgs e) {
    ...
    private void BeginUpdate(Control c) {
      ...
      OldEventMask =
          SendMessage(c.Handle, EM_SETEVENTMASK, IntPtr.Zero, IntPtr.Zero);
    }
    ...
    private void SaveSelectedObjectsToFile(int slot) {
      ...
      if (Globals.isMultiSelected) {
        ...
        selectedObjects.Add(level.getCurrentArea()
                                 .Objects[Globals.item_selected]
                                 .Data);
      }

      using (StreamWriter tempFile = new StreamWriter(
    ...
    private void LoadObjectsFromFile(int slot) {
      ...
      if (Globals.isMultiSelected) {
        ...
        int j = 0;
        for (int i = Globals.item_selected;
             i < level.getCurrentArea().Objects.Count &&
             j < objectsToLoad.Length;
             i++) {
