...
partial class ScriptDumps : Form {
  ...
  private void
      subTabsObjects_SelectedIndexChanged(object sender, EventArgs e) {
    refreshObjectsTabFast3DList();
  ...
  private void
      listBoxObjects_SelectedIndexChanged(object sender, EventArgs e) {
    refreshObjectsTabFast3DList();
    if (subTabsObjects.SelectedTab != null) {
      switch (subTabsObjects.SelectedIndex) {
        case 0:
  ...
  private void initObjectList() {
    ...
    objectCombos =
        level.LevelObjectCombos.GetRange(0, level.LevelObjectCombos.Count);
    ...
    for (int i = 0; i < level.Areas.Count; i++) {
      for (int j = 0; j < level.Areas[i].Objects.Count; j++) {
        ...
        if (combo_name.StartsWith("Undefined Combo (") &&
            !undefined_names.Contains(combo_name)) {
          ...
          ObjectComboEntry newOCE =
              new ObjectComboEntry(combo_name, modelID, modelAddress,
                                   behavior);
  ...
  private int findObjectComboInList(string name,
                                    uint behavior,
                                    uint geoLayout,
                                    byte modelID) {
    for (int i = 0; i < objectCombos.Count; i++) {
      if (objectCombos[i].Name == name &&
          objectCombos[i].Behavior == behavior &&
          objectCombos[i].ModelSegmentAddress == geoLayout &&
          objectCombos[i].ModelID == modelID) {
        return i;
      }
    }
    return -1;
  ...
  private void comboBoxObjectSort_SelectedIndexChanged(
      object sender,
      EventArgs e) {
    string comboName = "";
    ...
    if (listBoxObjects.SelectedIndex > -1) {
      ...
      geoLayoutAddress = objectCombos[listBoxObjects.SelectedIndex]
          .ModelSegmentAddress;
    ...
    if (changed)
      listBoxObjects.SelectedIndex =
          findObjectComboInList(comboName, behavior, geoLayoutAddress,
                                modelID);
  }
  ...
  private void updateObjectList() {
    switch (comboBoxObjectSort.SelectedIndex) {
      ...
      case 5:
        ...
        break;
      case 6:
        objectCombos = objectCombos
                       .OrderBy(
                           o => ROM.Instance.decodeSegmentAddress_safe(
                               o.Behavior, null))
                       .ToList();
        break;
    }
