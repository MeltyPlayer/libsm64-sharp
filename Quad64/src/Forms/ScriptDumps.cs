...
partial class ScriptDumps : Form {
  ...
  private RichTextBox currentTextbox,
                      lastTextBoxForLevelTab,
                      lastTextBoxForObjectsTab;
  ...
  private string format_long(byte[] cmd, int offset) {
    return string.Concat((bytesToInt(cmd, offset, 4)).ToString("X8"),
                         (bytesToInt(cmd, offset + 4, 4)).ToString("X8"),
                         " ");
  }
  ...
  private string format_threeBytesAsTwo(byte[] cmd, int offset) {
    ...
    return string.Concat(((tB >> 12) & 0xFFF).ToString("X3"), " ",
                         (tB & 0xFFF).ToString("X3"), " ");
  }


  [DllImport("user32.dll")]
  private static extern int SendMessage(IntPtr hwndLock,
                                        Int32 wMsg,
                                        Int32 wParam,
                                        ref Point pt);
  ...
  private void BeginUpdate(RichTextBox txtbox) {
    ...
    OldEventMask = SendMessage(txtbox.Handle, EM_SETEVENTMASK, IntPtr.Zero,
                               IntPtr.Zero);
  }
  ...
  private void updateScriptDump(object sender, EventArgs e) {
    ...
    if (currentTextbox.Name.EndsWith("_ls_textbox")) {
      ...
      if (sd_tabs.SelectedIndex == 0)
        ...
        ushort key = objectCombos[listBoxObjects.SelectedIndex].ModelID;
        if (level.ModelIDs.ContainsKey(key) &&
            ot_f3d_listbox.SelectedIndex > -1) {
          if (ot_f3d_listbox.SelectedItem.ToString().Equals("<Error 80>"))
            currentTextbox.Text =
                "Error: Quad64 cannot read display lists from segment 0.";
          else
      ...
      if (sd_tabs.SelectedIndex == 1 && listBoxObjects.SelectedIndex > -1) {
        ...
        // {
        List<ScriptDumpCommandInfo> behaviorDump =
            new List<ScriptDumpCommandInfo>();
        BehaviorScripts.parse(ref behaviorDump,
                              objectCombos[listBoxObjects.SelectedIndex]
                                  .Behavior);
  ...
  private void setupTheme() {
    ...
    listBoxObjects.BackColor =
        Theme.SCRIPTDUMPS_OBJECTSTAB_LISTBOX_BACKGROUND;
    ...
    lt_gls_radioPanel.BackColor =
        Theme.SCRIPTDUMPS_LEVELTAB_GEOLAYOUT_AREAPANEL_BACKGROUND;
    lt_f3d_radioPanel.BackColor =
        Theme.SCRIPTDUMPS_LEVELTAB_FAST3D_AREAPANEL_BACKGROUND;

    comboBoxObjectSort.BackColor =
        Theme.SCRIPTDUMPS_OBJECTSTAB_SORT_DROPDOWNLIST_BACKGROUND;
    comboBoxObjectSort.ForeColor =
        Theme.SCRIPTDUMPS_OBJECTSTAB_SORT_DROPDOWNLIST_TEXT;
  }
  ...
  private void listBoxObjects_DrawItem(object sender, DrawItemEventArgs e) {
    ...
    SolidBrush textBrush =
        new SolidBrush(Theme.SCRIPTDUMPS_OBJECTSTAB_LISTBOX_TEXT);
    if (isSelected) {
      e = new DrawItemEventArgs(e.Graphics,
                                ...
                                e.ForeColor,
                                Theme
                                    .SCRIPTDUMPS_OBJECTSTAB_LISTBOX_HIGHLIGHT);
      textBrush =
          new SolidBrush(
  ...
  private void lt_f3d_listbox_DrawItem(object sender, DrawItemEventArgs e) {
    ...
    bool isSelected =
        (e.State & DrawItemState.Selected) == DrawItemState.Selected;
    SolidBrush textBrush =
        new SolidBrush(Theme.SCRIPTDUMPS_FAST3D_LISTBOX_TEXT);
    if (isSelected) {
      e = new DrawItemEventArgs(e.Graphics,
                                e.Font,
                                e.Bounds,
      ...
      textBrush =
          new SolidBrush(Theme.SCRIPTDUMPS_FAST3D_LISTBOX_HIGHLIGHTEDTEXT);
    }
