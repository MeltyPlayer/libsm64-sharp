...
class TextureFormats {
  ...
  public static string ConvertFormatToString(byte formatColorType,
                                             byte formatByteSize) {
    string formatStr = "";
    ...
    switch (format) {
      ...
      case 0x48:
        throw new ArgumentException(
            "CI texture encoding is not currently supported in this version.");
    }
  }
  ...
  public static byte[] encodeIA4(Bitmap texture) {
    ...
    for (int i = 0; i < data_size; i++) {
      ...
      byte pix1_avg =
          (byte) ((((pix1.R + pix1.G + pix1.B) / 3) / 255.0f) * 8.0f);
      byte upper = (byte) ((pix1_avg << 1) | (pix1.A < 255 ? 0 : 1));

      Color pix2 = texture.GetPixel(x + 1, y);
      byte pix2_avg =
          (byte) ((((pix2.R + pix2.G + pix2.B) / 3) / 255.0f) * 8.0f);
  ...
  public static byte[] encodeIA8(Bitmap texture) {
    ...
    for (int i = 0; i < data_size; i++) {
      ...
      Color pix = texture.GetPixel(x, y);
      byte pix_avg =
          (byte) ((((pix.R + pix.G + pix.B) / 3) / 255.0f) * 16.0f);
  ...
  public static byte[] encodeI4(Bitmap texture) {
    ...
    for (int i = 0; i < data_size; i++) {
      ...
      Color pix1 = texture.GetPixel(x, y);
      byte upper =
          (byte) ((((pix1.R + pix1.G + pix1.B) / 3) / 255.0f) * 16.0f);

      Color pix2 = texture.GetPixel(x + 1, y);
      byte lower =
          (byte) ((((pix2.R + pix2.G + pix2.B) / 3) / 255.0f) * 16.0f);
  ...
  public static Bitmap decodeTexture(byte format,
                                     byte[] data,
                                     int width,
                                     int height,
                                     ushort[] palette,
