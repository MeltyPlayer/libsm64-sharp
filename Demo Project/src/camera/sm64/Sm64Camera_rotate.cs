...
namespace demo.camera.sm64 {
  public partial class Sm64Camera {
     ...
     */
    void rotate_in_xz(ref Vector3 dst, Vector3 src, short yaw) {
     ...
     */
    void rotate_in_yz(ref Vector3 dst, Vector3 src, short pitch) {
     ...
     * Rotates a vector in the horizontal plane and copies it to a new vector.
     */
    void rotatef_in_xz(ref Vector3 dst, Vector3 src, float yaw) {
      yaw *= 180 / MathF.PI;
     ...
     * space. If possible, use vec3f_set_dist_and_angle()
     */
    void rotatef_in_yz(ref Vector3 dst, Vector3 src, float pitch) {
      pitch *= 180 / MathF.PI;
